...
scramble_words
  ...
  let
    ...
    val @(pf_scrambled, pfgc_scrambled | p_scrambled) =
      array_ptr_alloc<String1> n
    macdef scrambled = !p_scrambled

    val () = array_initize_elt<String1> (scrambled, n, "*")
    val () = loop (scrambled, words_copy, letters, i2sz 0, 26)
    val scrambled_list = list_vt2t (array2list (scrambled, n))
    
    val () = array_ptr_free (pf_scrambled, pfgc_scrambled |
                             p_scrambled)
    val () = array_ptr_free (pf_words_copy, pfgc_words_copy |
                             p_words_copy)
  in
    scrambled_list
  end
...
verify_descrambled
          {n               : int}
          (words           : list (String1, n),
           scrambled_words : list (String1, n))
    : void =
  let
    prval () = lemma_list_param words
    val n = length words
    ...
    loop {m : nat}
         .<m>.
         (words           : list (String1, m),
          scrambled_words : list (String1, m))
        : void =
      case+ words of
      | list_nil () =>
        let
          val+ list_nil () = scrambled_words
        in
        end
      | list_cons (head1, tail1) =>
        let
          val+ list_cons (head2, tail2) = scrambled_words
        in
          if head1 <> head2 then
            begin
              println! ("Failed");
...
test_stability () : void =
  let
    ...
    implement
    list_stable_quicksort$lt<String1> (sx, sy) =
      (* Lexicographically compare first ASCII letters, in English
         order. *)
      tolower sx[0] < tolower sy[0]

    val scrambled_words = scramble_words words
    // val () = println! ($UN.cast{List string} scrambled_words)
    val sorted_words =
      list_vt2t (list_stable_quicksort<String1> words)
    // val () = println! ($UN.cast{List string} sorted_words)
    val () = verify_descrambled (words, sorted_words)
  in
  end
